...
DROP TABLE IF EXISTS dw.fact_saldos_financieros CASCADE;
DROP TABLE IF EXISTS dw.dim_tiempo         CASCADE;
DROP TABLE IF EXISTS dw.dim_entidad        CASCADE;
DROP TABLE IF EXISTS dw.dim_geografia      CASCADE;
DROP TABLE IF EXISTS dw.dim_producto       CASCADE;
DROP TABLE IF EXISTS dw.dim_tipo_operacion CASCADE;
DROP TABLE IF EXISTS dw.dim_fuente_datos   CASCADE;
...
COMMENT ON TABLE dw.dim_tiempo IS 'D1. Calendario de cortes mensuales. Jerarquía Año>Semestre>Trimestre>Mes.';
...
CREATE TABLE dw.dim_entidad (
    ...
    CONSTRAINT uq_entidad_version UNIQUE (cod_entidad, version)
);
...
COMMENT ON TABLE dw.dim_geografia IS
...
-- =====================================================================
CREATE TABLE dw.dim_tipo_operacion (
...
COMMENT ON TABLE dw.dim_tipo_operacion IS
...
COMMENT ON TABLE dw.dim_fuente_datos IS
...
-- =====================================================================
CREATE TABLE dw.fact_saldos_financieros (
    saldo_sk           BIGSERIAL PRIMARY KEY,
    -- ---- Claves foráneas a las 6 dimensiones ----
    tiempo_sk          INTEGER   NOT NULL REFERENCES dw.dim_tiempo(tiempo_sk),
    entidad_sk         INTEGER   NOT NULL REFERENCES dw.dim_entidad(entidad_sk),
    geografia_sk       INTEGER   NOT NULL REFERENCES dw.dim_geografia(geografia_sk),
    producto_sk        INTEGER   NOT NULL REFERENCES dw.dim_producto(producto_sk),
    tipo_operacion_sk  INTEGER   NOT NULL REFERENCES dw.dim_tipo_operacion(tipo_operacion_sk),
    fuente_sk          INTEGER   NOT NULL REFERENCES dw.dim_fuente_datos(fuente_sk),
    -- ---- Métricas comunes ----
    ...
    CONSTRAINT ck_saldo_no_negativo CHECK (saldo_total >= 0)
);

COMMENT ON TABLE dw.fact_saldos_financieros IS
...
CREATE INDEX ix_fact_tiempo    ON dw.fact_saldos_financieros (tiempo_sk);
CREATE INDEX ix_fact_entidad   ON dw.fact_saldos_financieros (entidad_sk);
CREATE INDEX ix_fact_geografia ON dw.fact_saldos_financieros (geografia_sk);
CREATE INDEX ix_fact_producto  ON dw.fact_saldos_financieros (producto_sk);
CREATE INDEX ix_fact_operacion ON dw.fact_saldos_financieros (tipo_operacion_sk);
CREATE INDEX ix_fact_fuente    ON dw.fact_saldos_financieros (fuente_sk);
CREATE INDEX ix_fact_cubo      ON dw.fact_saldos_financieros (tiempo_sk, tipo_operacion_sk, entidad_sk);
